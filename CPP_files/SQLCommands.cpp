...
void SQLCommands::executeCommand(const std::string &command) {
    ...
    if (PQresultStatus(res) != PGRES_COMMAND_OK) {
        ErrorMessages::printMessage("ERROR_EXECUTING_COMMAND");
...
PGresult *SQLCommands::executeQuery(PGconn *conn, const std::string &query) {
    if (!conn) {
        ErrorMessages::printMessage("CONNECTION_IS_NOT_ESTABLISHED");
    ...
    if (PQresultStatus(res) != PGRES_TUPLES_OK && PQresultStatus(res) != PGRES_COMMAND_OK) {
        ErrorMessages::printMessage("QUERY_EXECUTING_ERROR");
